import sys, sets
import lagrange_phylo, lagrange_newick, lagrange_ascii
import scipy

# This code comes from the lagrange package:
"""
...
##                 node.parent.age = node.age + node.length
    ...
    def calibrate(self, depth):
        ...
        for node in self.postorder_nodes:
            ...
            # kludge fix for when "node.lenth = None" for some reason (must be in newick file)
            if node.length == None:
                print ''
                print "kludge fix for when 'node.lenth = None' for some reason (must be in newick file)"
                print self
                #print self.postorder_nodes
                print "Old node.length:", node.length
                print scale
                    
                node.length = 0.01
                print "New node.length:", node.length
