...
class Tree(Nodes_v2.Chain):
    ...
    def root_with_outgroup(self,outgroup=None):
        ...
        root=Nodes_v2.Node(data=NodeData())            # new root    
...
def consensus(trees, threshold=0.5,outgroup=None):
    ...
    for (c,s) in clades.items():
        node=Nodes_v2.Node(data=dataclass())
        node.data.support=s
